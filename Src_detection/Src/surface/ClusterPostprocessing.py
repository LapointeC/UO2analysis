import os, re
import numpy as np
from .ASEVasp import WriteIncarASE
...
def ClearVaspInputs(work_path : str) -> None :
    """Clear Vasp inputs
...
def ClearVaspOutputs(work_path : str) -> None:
...
def PrintKPointsGrid(density : float, cell_param : np.ndarray, mode : str = 'fast') -> None :
...
def SetKpoints(density : float, cell_param : np.ndarray, speed : str) -> List[int]:
...
def SetKpointsSlab(density : float, cell_param : np.ndarray, speed : str, orientation_slab : List[int] = [0,0,1]) -> List[int]:
...
def SetupVaspASE(parser_vasp : AseVaspParser, input_path : str, work_path : str, density : float ,cell : np.ndarray, speed : str, el : str,**kwargs) -> int:
...
def SetupVaspSlabASE(parser_vasp : AseVaspParser, input_path : str, work_path : str, density : float, cell : np.ndarray, speed : str, slab_orientation : List[int],**kwargs) -> int :
    """Setup all VASP inputs for calculations 
...
def WritingSlurm(path_simu : str, full_slurm : str, name : str, nb_proc : int, kpar : int) -> None:

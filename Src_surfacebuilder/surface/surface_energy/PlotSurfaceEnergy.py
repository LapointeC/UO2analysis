...
class IntersectionObject : 
    ...
    def __init__(self, grid_MU1 : np.ndarray, 
                 grid_MU2 : np.ndarray, 
                 list_different_plane : List[str]) -> None : 
    ...
    def build_intersection_vector(self, plane1 : str, 
                                  plane2 : str, 
                                  method : str = 'least_square') -> np.ndarray : 
    ...
    def find_intersection_point(self, plane1 : str, 
                                plane2 : str, 
                                nearest_plane : List[str], 
    ...
    def build_plane_intersection_dictionnary(self, nearest_plane : str, 
    ...
    def apply_constraint(self,mu1_l : np.ndarray, 
                         mu2_l : np.ndarray, 
                         plane : str) -> Tuple[np.ndarray,np.ndarray]: 
...
def ReadFileNormalVector(file : os.PathLike[str]) -> Tuple[Dict[str,np.ndarray], Dict[str,str], Dict[str,Dict[str,List[float]]], Dict[str,List[any]]] :
...
def PlotHyperplane(dic_vect : Dict[str,np.ndarray], 
                   dic_mu : Dict[str,str],
                   dic_bounds : Dict[str,Dict[str,List[float]]], 
                   slab : str, 
                   color : str, 
...
def PlotAllHyperplanes3D(file_to_read : os.PathLike[str], 
                         slab_to_plot : List[str]) -> None : 
...
def ComputeDistanceToHyperplane(vect : np.ndarray, 
...
def ComputeNearestHyperplane(list_slab : List[str], 
                             list_normal_vect : List[np.ndarray], 
                             mu_vect : np.ndarray) -> Tuple[str,float] :
...
def RecursiveMuGrid(list_vect : List[np.ndarray], 
                    list_id : List[int], 
                    dimension : int, 
                    coordinate : List[List[float]] = [], 
                    compt_dim : int=0) -> None : 
...
def RelativeStabilityOnGrid(dic_bounds : Dict[str,Dict[str,List[float]]], 
                            list_slab : List[str], 
                            dic_vect : Dict[str,np.ndarray], 
                            dic_constraint : Dict[str,List[List[str] | np.ndarray]],
                            discr_per_mu : int = 100) -> None :
...
def RelativeStabilityOnGrid2DPlot(dic_bounds : Dict[str,Dict[str,List[float]]], 
                                  list_slab : List[str], 
                                  dic_vect : Dict[str,np.ndarray], 
                                  dic_constraint : Dict[str,List[List[str] | np.ndarray]],
                                  discr_per_mu : int = 100) -> Tuple[np.ndarray,np.ndarray,np.ndarray,IntersectionObject] :
    """Compute the relative stability (in term of probabilities) between several slab system by taking into account constraints on chemical
...
def PlotProjectionHyperplaneInto2D(file_to_read : os.PathLike[str], 
                                   slab_to_plot : List[str], 
                                   grid_mu : int, 
                                   level_line : bool = False) -> None : 

...
def CheckSurfaceCompacity(slab: Atoms, 
                          list_atom_surface: List[Atom], 
                          tolerance_compacity: float, 
                          compacity_ref: float) -> bool:
...
def BuildDenseHighTermination(composition: List[str], 
                              aseslab: Atoms, 
                              ...
                              tolerance_compacity: float, 
                              compacity_ref: float, 
                              remesher: DictRemesh) -> Tuple[Atoms, bool]:
...
def CompositionFromBulk(file: os.PathLike[str]) -> List[str]:
    ...
    """
    bulk = io.read(file)
    return np.unique(bulk.get_chemical_symbols()).tolist()

def BuildCompositionDistanceVectorSlab(composition: List[str], 
                                       slab: Atoms, 
                                       terminaison : str = 'haut', 
                                       tolerance : float = 1e-2, 
                                       sym_check: DictSym = {'bool':False,'replica':1, 'rcut':3.0}, alpha_list: List[float] = None, remesher: DictRemesh = {'bool': False, 'rcut': 0.0, 'beta': 1.0}) -> Tuple[List[float], List[float], List[Atom]]:
    ...
    mask_tol = np.abs(slab.positions[:,2] - extremum_z_position) < tolerance
    ats_terminaison = slab[mask_tol]
    list_idx = np.arange(len(slab))[mask_tol]
    list_atom_surface = [slab[id] for id in list_idx]

    list_position = ats_terminaison.positions
    for at in ats_terminaison :
        list_compo[composition.index(at.symbol)] += 1
    sum_compo = np.sum(list_compo)
    ...
    # Generate replica shifts efficiently using NumPy
    e_range = np.arange(-int((repli - 1) / 2), int((repli - 1) / 2) + 1)
    replica_shifts = np.array(np.meshgrid(e_range, e_range)).T.reshape(-1, 2) @ cell.T  # (num_replicas, 2)
    replica_shifts = np.concatenate((replica_shifts, np.zeros((replica_shifts.shape[0],1))), axis=1)

    # Compute replicated positions using broadcasting
    #list_position = list_position  # Convert to NumPy array
    list_position_replicated = (list_position[:, None, :] + replica_shifts).reshape(-1, 3)  # (M * N, 2)

    # Normalize composition list
    list_compo = (np.array(list_compo) / sum_compo).tolist()

    # Compute distances using NumPy broadcasting
    pos_i_exp = list_position[:, np.newaxis, :]  # Shape (M, 1, 3)
    pos_j_exp = list_position_replicated[np.newaxis, :, :]  # Shape (1, N, 3)
    distances = np.linalg.norm(pos_i_exp - pos_j_exp, axis=2)  # Shape (M, N)

    # Apply cutoff filter
    mask_rcut = distances < rcut 
    mask_nul = distances > 0

    valid_distances = distances[mask_rcut & mask_nul]

    if alpha_list is None:
        sum_distance = valid_distances.sum()/(len(valid_distances)**2)
    else:
        sum_distance = np.zeros(len(alpha_list) + 1)
        sum_distance[0] = valid_distances.sum()
        for id_alpha, alpha in enumerate(alpha_list):
            sum_distance[id_alpha + 1] = np.sum(valid_distances ** alpha)/(len(valid_distances)**2)

    return list_compo, sum_distance.tolist(), list_atom_surface


def AtomDeleter(slab: Atoms, 
                list_index: List[Atom], 
                copy : bool = True) -> Atoms:
...
def DeepLevelTerminaison(niveau: int, 
                         composition: List[str], 
                         slab: Atoms, 
                         terminaison : str ='haut', 
                         tolerance : float =1e-2, 
                         sym: DictSym  = {'bool':False, 'replica':1, 'rcut':3.0}, 
                         alpha_list : List[float] = None, 
                         remesher={'bool': False, 'rcut': 0.0, 'beta': 1.0}) -> Tuple[List[List[float]], List[List[float]]]:

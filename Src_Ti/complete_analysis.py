...
class Dfct_analysis_object : 
    ...
    def update_dfct(self, key_dfct : str, atom : Atom, array_property : Dict[str,Any] = {}, rcut : float = 4.0, elliptic : str = 'iso') :
        ...
        if self.dfct[key_dfct] == {} : 
            ...
            key_to_return, min_dist = None, 1e4
            for id_key in self.dfct[key_dfct].keys() : 
                center_dfct = self.dfct[key_dfct][id_key].center
                distance = np.linalg.norm(atom.position - center_dfct)
                if self.dfct[key_dfct][id_key].get_elliptic_distance(atom) < 1.5 : 
                    if distance < min_dist : 
                        min_dist = distance
                        key_to_return = id_key

            if key_to_return is not None : 
                self.dfct[key_dfct][key_to_return].append(atom, array_property=array_property, elliptic = elliptic)
            else : 

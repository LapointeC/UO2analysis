...
class BaseWorker:
    ...
    def sampling_Wigner_process(self,temperature : float, delta_t : float, shape : tuple) -> np.ndarray : 
        """Generating Wigner noise for stochastic dynamics
        ...
        return np.random.normal(0,np.sqrt(2*self.kB*temperature*delta_t),shape)

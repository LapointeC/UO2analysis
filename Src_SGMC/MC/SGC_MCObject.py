...
class SGCMC : 
    def __init__(self, mu_array : np.ndarray, n_species_array : np.ndarray, lammps_worker : LAMMPSWorker) -> None :
    ...
    def compute_deltaE(self, id_atom : int, new_species : str | int) -> Tuple[float,int|str] : 
        ...
        #new energy
        self.worker.run_commands("run 0 pre no post no")
    ...
    def perform_SGCMC(self, fraction_swap : float, temperature : float) -> float : 
        ...
        for _ in range(int(fraction_swap*self.Natom)) : 
            ...
            #a = self.get_number_of_atoms_each_species()
            #if self.worker.rank == 0 :
            #    print(np.array(self.n_species_array),species_to_test)
            #    print(a)
    ...
    def dump_configuration(self, name_configuration : str) -> None :
        ...
        """
        self.worker.run_commands(f'write_dump all custom  test_{name_configuration}.dump id type x y z')
        return 
...
class VC_SGCMC : 
    def __init__(self, mu_array : np.ndarray, concentration_array : np.ndarray, n_species_array : np.ndarray, kappa : float, lammps_worker : LAMMPSWorker) -> None :

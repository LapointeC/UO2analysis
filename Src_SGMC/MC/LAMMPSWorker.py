...
class LAMMPSWorker(BaseWorker):
    ...
    def start_lammps(self)->None:
        ...
        """
        if self.parameters.parameters["LogLammps"]:
    ...
    def run_script(self,key:str,arguments:None|dict=None)->None:
        ...
        if key in self.parameters.scripts:
            if self.parameters.parameters["Verbose"]>0 and self.rank==0:
    ...
    def extract_data(self, name : str, type: None|int = None) -> np.ndarray : 
        return self.L.extract_atom(name,type)
